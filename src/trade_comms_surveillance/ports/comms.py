...
Cmp1 does NOT redeclare the speech boundary: the transcription and diarization ports, the
transcript and speaker-turn types, and the redaction spans all come from the pinned
...
consumer has a single import site for the comms boundary, exactly as ``ports/__init__.py`` gives
the rest of the hexagon one.
...
ports: nothing in Cmp1's request path calls a live recogniser.
"""
